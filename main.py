...
class PlannerApp:
    ...
    def build_reminders_tab(self):
        ...
        ttk.Label(frame, text=t("reminders.telegram_title"), font=("Arial", 13, "bold")).pack(anchor="w")
        ttk.Label(frame, text=self._status_line(config.telegram_ready())).pack(anchor="w", pady=(2, 6))
        ttk.Button(frame, text=t("reminders.test_telegram"), command=self.test_telegram).pack(anchor="w", pady=(0, 24))

        ttk.Label(frame, text=t("reminders.email_title"), font=("Arial", 13, "bold")).pack(anchor="w")
        ttk.Label(frame, text=self._status_line(config.email_ready())).pack(anchor="w", pady=(2, 6))
        ttk.Button(frame, text=t("reminders.test_email"), command=self.test_email).pack(anchor="w", pady=(0, 24))

        self.reminders_status_label = ttk.Label(frame, text="", font=("Arial", 10, "bold"))
        self.reminders_status_label.pack(anchor="w")

        ttk.Label(frame, text=t("reminders.env_note"), foreground="#7F8C8D", wraplength=760).pack(
            anchor="w", pady=(20, 6)
        )
        ttk.Label(frame, text=t("reminders.cron_note"), foreground="#7F8C8D", wraplength=760).pack(anchor="w")

    @staticmethod
    def _status_line(ready: bool) -> str:
        return t("reminders.configured") if ready else t("reminders.not_configured")

    def test_telegram(self):
        if not config.telegram_ready():
            messagebox.showwarning(t("msg.error"), t("reminders.missing_env"))
            return
